...
- `httpOnly`, `sameSite=strict`, `secure` in production, `path=/`, three days —
  the same three days as the token it carries, from the backend's
  `SESSION_TTL_SECONDS`. A cookie outliving its token would only leave a
  coordinator looking signed in while every request came back 401.
